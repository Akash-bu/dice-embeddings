...
BATCH_SIZE = "1024"
# Grid search values: every (learning rate, embedding dim) combination is run.
LEARNING_RATES = ["0.1", "0.01", "0.001"]
EMB_DIMS = ["32", "64", "128"]
NUM_EPOCHS = "100"
SCORING_TECH = "KvsAll"
OPTIM = "Adam"
# Grid search selects hyperparameters on the validation set (keeps test untouched).
# "train_val" also reports train metrics per run so you can watch the train/val gap.
EVAL_MODEL = "train_val"
...
def _dataset_root_for(loss_function, project_root):
    """Return the Datasets_Perturbed root appropriate for the given loss."""
    if loss_function in PCRA_LOSSES:
        return project_root / "Datasets_Perturbed_pcra"
    return project_root / "Datasets_Perturbed"

def create_results_table(results_dict):
    # Structure: results_dict[dataset][model][loss][scoring][neg_ratio][lr][emb_dim] = metrics
    rows = []
        ...
        for model, losses_dict in models_dict.items():
            for loss, scoring_dict in losses_dict.items():
                for scoring, neg_dict in scoring_dict.items():
                    for neg_ratio, lr_dict in neg_dict.items():
                        for lr, emb_dict in lr_dict.items():
                            for emb_dim, metrics in emb_dict.items():
                                metrics = metrics or {}
                                rows.append({
                                    'Dataset': dataset,
                                    'Model': model,
                                    ...
                                    'LR': lr,
                                    'Emb_Dim': emb_dim,
                                    'Val_MRR': metrics.get('MRR'),
                                    'Val_H@1': metrics.get('H@1'),
                                    'Val_H@3': metrics.get('H@3'),
                                    'Val_H@10': metrics.get('H@10'),
                                })

    df = pd.DataFrame(rows)

    # Pivot: models as rows, (Dataset, Loss, Scoring, Neg_Ratio, LR, Emb_Dim) as columns
    pivot_df = df.pivot_table(
        index='Model',
        columns=['Dataset', 'Loss', 'Scoring', 'Neg_Ratio', 'LR', 'Emb_Dim'],
        values='Val_MRR',
        aggfunc='first'
    )
    pivot_df = pivot_df.sort_index(axis=1, level=[0, 1, 2, 3, 4, 5])
...
def _format_inner_label(col):
    ...
    if len(parts) == 5:
        # Loss, Scoring, Neg_Ratio, LR, Emb_Dim
        return f"{parts[0]}/{parts[1]}\n$n={parts[2]}$\nlr={parts[3]}, d={parts[4]}"
    if len(parts) == 3:
        return f"{parts[0]}/{parts[1]}\n$n={parts[2]}$"
    if len(parts) == 2:
        return f"{parts[0]}/{parts[1]}"
    ...
    with plt.rc_context(pub_rc):
        ...
        for i, (ax, ds) in enumerate(zip(axes, datasets)):
            ...
            sns.heatmap(
                ...
                cbar_kws={"label": "Val MRR"} if show_cbar else None,
        ...
        suptitle = f"Val MRR — configurations: {config_name}"
...
def save_results_table(df, pivot_df, output_dir, configs=None, models=None, neg_ratios=None):
    ...
    print("\n" + "="*80)
    print("MRR Comparison Table (Validation Set)")
    print("="*80)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--lr', type=str, nargs='+', default=None,
    help = 'One or more learning rates to grid-search (e.g., --lr 0.1 0.01 0.001). '
           'If not provided, uses LEARNING_RATES from config.py')
    ...
    parser.add_argument('--emb_dim', type=str, nargs='+', default=None,
    help = 'One or more embedding dimensions to grid-search (e.g., --emb_dim 32 64 128). '
           'If not provided, uses EMB_DIMS from config.py')
    ...
    # Learning rates / embedding dims are grid-searched: always lists.
    if args.lr:
    ...
    if args.emb_dim:
        embedding_dims = args.emb_dim
    elif isinstance(EMB_DIMS, (list, tuple)):
        embedding_dims = list(EMB_DIMS)
    else:
    ...
    # dicee reports metrics under 'Train'/'Val'/'Test' keys depending on eval_model
    # (substring match). Pick the split we're selecting on for the results table.
    if 'test' in eval_model:
        report_key = 'Test'
    elif 'val' in eval_model:
        report_key = 'Val'
    elif 'train' in eval_model:
        report_key = 'Train'
    else:
        report_key = 'Test'
    ...
    for DB in DBS:
        ...
        for root in used_roots:
            ...
            if db_path.exists():
                ...
                print(f"Warning: {db_path} does not exist, skipping it for {DB}")
        if not subdir_set:
            print(f"Warning: no dataset root found for {DB}, skipping")
            continue
            ...
            for MODEL in models_to_run:
                ...
                for loss_function, scoring_technique in loss_scoring_pairs:
                    ...
                    if not dataset_folder.exists():
                        print(
                            f"Warning: {dataset_folder} does not exist, skipping "
                            f"{MODEL} on {dataset_name} with loss={loss_function}"
                        )
                        for neg_ratio in effective_neg_ratios:
                            for lr in learning_rates:
                                for emb_dim in embedding_dims:
                                    all_results[dataset_name][MODEL].setdefault(loss_function, {}).setdefault(scoring_technique, {}).setdefault(neg_ratio, {}).setdefault(lr, {})[emb_dim] = None
                        continue
                    for neg_ratio in effective_neg_ratios:
                        # Grid search over (learning rate, embedding dim).
                        for lr in learning_rates:
                            for emb_dim in embedding_dims:
                                tag = (
                                    f"loss={loss_function} scoring={scoring_technique} "
                                    f"neg_ratio={neg_ratio} lr={lr} emb_dim={emb_dim}"
                                )
                                ...
                                try:
                                    result = run_dicee_eval(
                                        ...
                                        path_to_store_single_run=str(
                                            results_dir / DB / subdir / MODEL / f"{loss_function}_{scoring_technique}_neg{neg_ratio}_lr{lr}_d{emb_dim}" / ""
                                        ),
                                    ...
                                    eval_metrics = result.get(report_key, {})

                                    all_results[dataset_name][MODEL].setdefault(loss_function, {}).setdefault(scoring_technique, {}).setdefault(neg_ratio, {}).setdefault(lr, {})[emb_dim] = eval_metrics
                                    print(f"Completed: {MODEL} on {dataset_name} {tag} - {report_key} MRR: {eval_metrics.get('MRR')}")
