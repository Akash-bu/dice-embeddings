                ...
                for scoring, neg_dict in scoring_dict.items():
                    for neg_ratio, metrics in neg_dict.items():
                        ...
                        rows.append({
                            ...
                            'Test_H@1_std': metrics.get('H@1_std'),
                            'Test_H@3': metrics.get('H@3'),
                            'Test_H@3_std': metrics.get('H@3_std'),
                            'Test_H@10': metrics.get('H@10'),
                            'Test_H@10_std': metrics.get('H@10_std'),
                        })
...
def create_metrics_table(df):
    ...
    """
    metric_cols = ['Test_MRR', 'Test_H@1', 'Test_H@3', 'Test_H@10']
...
def _safe_suffix(suffix, max_len=100):
...
def save_results_table(df, pivot_df, output_dir, configs=None, models=None, neg_ratios=None, kind=None, dataset = None):
    ...
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    suffix = _safe_suffix(
        f"_{dataset}_{_models_suffix(models)}{_loss_suffix(configs)}"
    )
    if kind:
...
if __name__ == "__main__":
    ...
    results_dir = robust_kge_dir / "saved_models_bo_rand"
    ...
    if all_results:
        ...
        save_results_table(
            df,
            pivot_df,
            project_root / "robust-kge" / "Random_perturbed_seed" / "results",
            loss_scoring_pairs,
            models_to_run,
            neg_ratios,
            kind="random",
